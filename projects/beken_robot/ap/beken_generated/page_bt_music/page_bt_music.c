 ...
 *   - a cyan/purple "glass" spectrum panel with lightweight accent lines and
 *     8 rainbow bars;
...
#define BTM_IMM_BASE_Y    (BTM_IMM_BAR_TOP + BTM_IMM_BAR_H)
 #define BTM_ENTRY_BLOCK_HEAP (36U * 1024U)
 #define BTM_LOW_RENDER_HEAP  (48U * 1024U)
...
static lv_obj_t  *s_imm_layer;
static lv_obj_t  *s_imm_bar[BTM_BARS];
static lv_obj_t  *s_imm_cap[BTM_BARS];
...
static lv_obj_t *create_imm_bar(lv_obj_t *parent, int x, int idx)
{
    lv_obj_t *bar = lv_bar_create(parent);
    lv_obj_set_pos(bar, x, BTM_IMM_BAR_TOP);
    lv_obj_set_size(bar, BTM_IMM_BAR_W, BTM_IMM_BAR_H);
    lv_bar_set_range(bar, 0, 100);
    lv_bar_set_value(bar, BTM_BAR_MIN, LV_ANIM_OFF);
    lv_obj_set_style_bg_color(bar, lv_color_hex(C_BAR_TRACK), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(bar, LV_OPA_50, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(bar, 8, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(bar, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(bar, lv_color_hex(s_bar_hue[idx]), LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(bar, LV_OPA_COVER, LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(bar, 8, LV_PART_INDICATOR | LV_STATE_DEFAULT);
    return bar;
}

static lv_obj_t *create_imm_cap(lv_obj_t *parent, int x, int idx)
{
    lv_obj_t *cap = lv_obj_create(parent);
    lv_obj_remove_style_all(cap);
    lv_obj_set_size(cap, BTM_IMM_BAR_W, 3);
    lv_obj_set_pos(cap, x, BTM_IMM_BASE_Y - BTM_BAR_MIN - 3);
    lv_obj_set_style_bg_color(cap, lv_color_hex(lighten(s_bar_hue[idx], 60)),
                              LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(cap, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(cap, 2, LV_PART_MAIN | LV_STATE_DEFAULT);
    return cap;
}
 ...
 static lv_obj_t *create_cap(lv_obj_t *parent, int x, int idx)
 {
     ...
     lv_obj_set_style_bg_color(cap, lv_color_hex(lighten(s_bar_hue[idx], 55)),
                               LV_PART_MAIN | LV_STATE_DEFAULT);
     lv_obj_set_style_bg_opa(cap, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
     lv_obj_set_style_radius(cap, 1, LV_PART_MAIN | LV_STATE_DEFAULT);
     return cap;
 }
                                   ...
                                   lv_event_cb_t cb, lv_obj_t **out_label)
 {
     ...
     if (primary) {
         ...
         lv_obj_set_style_bg_opa(btn, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
         lv_obj_set_style_border_color(btn, lv_color_hex(C_BTN_BORDER), LV_PART_MAIN | LV_STATE_DEFAULT);
...
static bool create_immersive_layer(void)
{
    if (s_imm_layer != NULL && lv_obj_is_valid(s_imm_layer)) {
        return true;
    }
    if (s_screen == NULL || !lv_obj_is_valid(s_screen) || s_low_render_mode) {
        return false;
    }

    s_imm_layer = lv_obj_create(s_screen);
    lv_obj_remove_style_all(s_imm_layer);
    lv_obj_set_size(s_imm_layer, LOGICAL_SCREEN_WIDTH, LOGICAL_SCREEN_HEIGHT);
    lv_obj_set_pos(s_imm_layer, 0, 0);
    lv_obj_set_style_bg_color(s_imm_layer, lv_color_hex(C_BG_BOTTOM),
                              LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(s_imm_layer, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_add_event_cb(s_imm_layer, on_any_press, LV_EVENT_PRESSED, NULL);
    lv_obj_add_flag(s_imm_layer, LV_OBJ_FLAG_HIDDEN);

    lv_obj_t *base = lv_obj_create(s_imm_layer);
    lv_obj_remove_style_all(base);
    lv_obj_set_size(base, LOGICAL_SCREEN_WIDTH - 48, 3);
    lv_obj_set_pos(base, 24, BTM_IMM_BASE_Y + 5);
    lv_obj_set_style_bg_color(base, lv_color_hex(C_BASE_GLOW), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(base, LV_OPA_50, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(base, 2, LV_PART_MAIN | LV_STATE_DEFAULT);

    const int span = BTM_BARS * BTM_IMM_BAR_W + (BTM_BARS - 1) * BTM_IMM_BAR_GAP;
    const int x0 = (LOGICAL_SCREEN_WIDTH - span) / 2;
    for (int i = 0; i < BTM_BARS; i++) {
        int x = x0 + i * (BTM_IMM_BAR_W + BTM_IMM_BAR_GAP);
        s_imm_bar[i] = create_imm_bar(s_imm_layer, x, i);
        s_imm_cap[i] = create_imm_cap(s_imm_layer, x, i);
    }
    return true;
}
...
{
    if (enable) {
        if (!create_immersive_layer()) {
            return;
        }
        s_immersive = true;
        lv_obj_remove_flag(s_imm_layer, LV_OBJ_FLAG_HIDDEN);
        lv_obj_move_foreground(s_imm_layer);
    } else {
        s_immersive = false;
        if (s_imm_layer != NULL && lv_obj_is_valid(s_imm_layer)) {
            lv_obj_add_flag(s_imm_layer, LV_OBJ_FLAG_HIDDEN);
 ...
 static void on_play_pause(lv_event_t *e)
 {
     (void)e;
     bt_music_play_pause();
     s_ui_playing = !s_ui_playing;
     set_play_symbol(s_ui_playing);
 }
 ...
 static void meter_timer_cb(lv_timer_t *timer)
 {
    ...
    bool playing = bt_music_is_playing();
     ...
     for (int i = 0; i < BTM_BARS; i++) {
        ...
        if (s_immersive && s_imm_bar[i] != NULL && lv_obj_is_valid(s_imm_bar[i])) {
            lv_bar_set_value(s_imm_bar[i], (int)(s_disp[i] + 0.5f), LV_ANIM_OFF);
        }
         ...
         if (s_cap[i] != NULL && lv_obj_is_valid(s_cap[i])) {
             int ph = (int)(s_peak[i] / 100.0f * (float)BTM_BAR_H + 0.5f);
             int cap_y = BTM_BASE_Y - ph - BTM_CAP_H;
             if (cap_y < BTM_BAR_TOP) {
                 cap_y = BTM_BAR_TOP;
             }
             lv_obj_set_y(s_cap[i], cap_y);
         }
        if (s_immersive && s_imm_cap[i] != NULL && lv_obj_is_valid(s_imm_cap[i])) {
            int ph = (int)(s_peak[i] / 100.0f * (float)BTM_IMM_BAR_H + 0.5f);
            int cap_y = BTM_IMM_BASE_Y - ph - 3;
            if (cap_y < BTM_IMM_BAR_TOP) {
                cap_y = BTM_IMM_BAR_TOP;
            }
            lv_obj_set_y(s_imm_cap[i], cap_y);
        }
     }
 ...
 int page_bt_music_enter(void)
 {
    ...
    s_last_touch_ms = (uint32_t)rtos_get_time();
     ...
     s_link_dot = lv_obj_create(s_screen);
     ...
     s_link_label = lv_label_create(s_screen);
     ...
     lv_obj_t *base = lv_obj_create(s_screen);
     lv_obj_remove_style_all(base);
     ...
     /* ---- music transport row: VOL- PREV PLAY NEXT VOL+ (compact, centered) ---- */
     {
         ...
         int yn = 216, yp = 211;
         create_round_btn(s_screen, LV_SYMBOL_VOLUME_MID, rx,                             yn, dn, false, on_vol_down, NULL);
         create_round_btn(s_screen, LV_SYMBOL_PREV,       rx + (dn + gap),                yn, dn, false, on_prev, NULL);
         create_round_btn(s_screen, LV_SYMBOL_PLAY,       rx + 2 * (dn + gap),            yp, dp, true,  on_play_pause, &s_play_label);
         create_round_btn(s_screen, LV_SYMBOL_NEXT,       rx + 2 * (dn + gap) + dp + gap, yn, dn, false, on_next, NULL);
         create_round_btn(s_screen, LV_SYMBOL_VOLUME_MAX, rx + 3 * (dn + gap) + dp + gap, yn, dn, false, on_vol_up, NULL);
     }
     create_robot_btn(s_screen, 54, 274, 276, 34, on_robot_toggle);
     refresh_hand();
 
     s_meter_timer = lv_timer_create(meter_timer_cb, 33, NULL);
     if (s_meter_timer != NULL) {
