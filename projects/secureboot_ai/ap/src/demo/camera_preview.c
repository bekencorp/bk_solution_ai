...
//
// Note on the "no LVGL in src/demo/" rule: camera preview is an overlay
// demo that takes the framebuffer away from LVGL (ARGB8888 panel + GPU
// composer), so the lifecycle inherently has to coordinate with the LVGL
// vendor (pause on enter, resume + invalidate on exit). lv_vendor.h is
// admitted here as a documented exception, mirroring palm_tracking.cc.
// Page-bound demos (asr, music, volume, ...) stay strictly LVGL-free.
 ...
 *
 * The SP channel free-runs into a small ring (APP_ISP_SP_BUF_CNT deep in
 ...
 * so once the buffers fill the ISP stalls holding STALE frames captured
 * right after the PREVIOUS shot. media_camera_sp_read() (pop_buf) then
 * returns the OLDEST of those, so the JPEG handed to the vision LLM shows
 * the previous scene, not the frame the user just froze on screen (the GPU
 * display snapshot is a *different*, current frame). Root cause of
 * BK7259SW-3214 ("AI analyzes the previous frozen photo").
 *
...
/* ----------------------------------------------------------------------
 * bk_demo_iface_t wiring (merged in from the old src/demo/camera_preview_demo.c
 * thin wrapper). The menu dispatch table in demo_registry references
 * camera_preview_demo_start; keep the name stable for backward compatibility.
 * -------------------------------------------------------------------- */

int camera_preview_demo_init(void) { return 0; }
...
const bk_demo_iface_t g_demo_camera_preview = {
    "camera_preview",
    camera_preview_demo_init,
    camera_preview_demo_start,
    camera_preview_demo_stop,
};
